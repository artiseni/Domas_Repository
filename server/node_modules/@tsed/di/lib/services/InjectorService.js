...
let InjectorService = InjectorService_1 = class InjectorService extends Container_1.Container {
     ...
     */
    scopeOf(provider) {
        return provider.scope || this.scopes[provider.type] || domain_1.ProviderScope.SINGLETON;
    }
    /**
    ...
    get(token, options = {}) {
        var _a;
        const instance = (_a = super.get(core_1.getClassOrSymbol(token))) === null || _a === void 0 ? void 0 : _a.instance;
     ...
     */
    has(token) {
        return super.has(core_1.getClassOrSymbol(token)) && this.get(token) !== undefined;
    }
    /**
    ...
    invoke(token, locals = new LocalsContainer_1.LocalsContainer(), options = {}) {
        ...
        switch (this.scopeOf(provider)) {
            case domain_1.ProviderScope.SINGLETON:
                if (!this.has(token)) {
                ...
                break;
            case domain_1.ProviderScope.REQUEST:
                instance = this.resolve(token, locals, options);
                locals.set(token, instance);
                break;
            case domain_1.ProviderScope.INSTANCE:
    ...
    loadSync(locals = new LocalsContainer_1.LocalsContainer()) {
        for (const [, provider] of this) {
            if (!locals.has(provider.token) && this.scopeOf(provider) === domain_1.ProviderScope.SINGLETON) {
     ...
     * @param rootModule
     */
    async load(container = createContainer_1.createContainer(), rootModule) {
    ...
    resolveConfiguration() {
        ...
        super.forEach((provider) => {
            if (provider.configuration) {
                Object.entries(provider.configuration).forEach(([key, value]) => {
                    value = mergedConfiguration.has(key) ? core_1.deepMerge(mergedConfiguration.get(key), value) : core_1.deepClone(value);
        ...
        mergedConfiguration.forEach((value, key) => {
            this.settings[key] = core_1.deepMerge(value, this.settings[key]);
        });
        this.settings.build();
    ...
    bindInjectableProperties(instance, locals, options) {
        ...
        Object.values(properties).forEach((definition) => {
            switch (definition.bindingType) {
                case domain_1.InjectablePropertyType.METHOD:
                    this.bindMethod(instance, definition);
                    break;
                case domain_1.InjectablePropertyType.PROPERTY:
                    this.bindProperty(instance, definition, locals, options);
                    break;
                case domain_1.InjectablePropertyType.CONSTANT:
                    this.bindConstant(instance, definition);
                    break;
                case domain_1.InjectablePropertyType.VALUE:
                    this.bindValue(instance, definition);
                    break;
                case domain_1.InjectablePropertyType.INTERCEPTOR:
     ...
     * @param invokeOptions
     */
    bindProperty(instance, { propertyKey, useType, onGet = (f) => f, options }, locals, invokeOptions) {
    ...
    mapInvokeOptions(token, locals, options) {
        ...
        return {
            token,
            scope: scope || core_1.Store.from(token).get("scope") || domain_1.ProviderScope.SINGLETON,
            deps: deps || [],
            imports: imports || [],
            isBindable,
            construct,
...
InjectorService = InjectorService_1 = tslib_1.__decorate([
    injectable_1.Injectable({
        scope: domain_1.ProviderScope.SINGLETON,
        global: true
    }),
