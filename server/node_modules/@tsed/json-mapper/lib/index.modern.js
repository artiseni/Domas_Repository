...
import { isBoolean, objectKeys, isCollection, isEmpty, isArray, nameOf, isNil, isClass, classOf, isPlainObject, isFunction } from '@tsed/core';
import { BadRequest } from '@tsed/exceptions';
import { JsonEntityFn, JsonEntityStore, getProperties, alterIgnore, JsonSchema, getPropertiesStores } from '@tsed/schema';
...
let ArrayMapper = class ArrayMapper {
...
ArrayMapper = __decorate([JsonMapper(Array)], ArrayMapper);
...
DateMapper = __decorate([JsonMapper(Date)], DateMapper);
...
MapMapper = __decorate([JsonMapper(Map)], MapMapper);
...
PrimitiveMapper = __decorate([JsonMapper(String, Number, Boolean)], PrimitiveMapper);
...
SetMapper = __decorate([JsonMapper(Set)], SetMapper);
...
SymbolMapper = __decorate([JsonMapper(Symbol)], SymbolMapper);
...
function plainObjectToClass(src, options) {
  ...
  const {
    type,
    store = JsonEntityStore.from(type)
  } = options,
        next = _objectWithoutPropertiesLoose(options, _excluded3);

  const propertiesMap = getProperties(store, _extends({}, options, {
...
function classToPlainObject(obj, options) {
  ...
  return getSchemaProperties(entity, obj).reduce((newObj, [key, propStore]) => {
    ...
    if (alterIgnore(schema, _extends({
      useAlias
    }, props, {
      self: obj
    }))) {
    ...
    let value = alterValue(schema, obj[key], _extends({
      useAlias
    }, props, {
      self: obj
    }));
...
export { AfterDeserialize, ArrayMapper, BeforeDeserialize, DateMapper, JsonMapper, JsonMapperContext, MapMapper, OnDeserialize, OnSerialize, PrimitiveMapper, SetMapper, SymbolMapper, classToPlainObject, deserialize, getJsonMapperTypes, plainObjectToClass, registerJsonTypeMapper, serialize };
