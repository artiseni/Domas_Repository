...
function getStatusConstant(status) {
  return changeCase.constantCase(HTTP_STATUS_MESSAGES[status]);
}
...
function getJsonEntityStore(...args) {
  const store = core.Store.from(...args);

  if (!store.has("JsonEntityStore")) {
    const entityStore = JsonEntitiesContainer.get(core.decoratorTypeOf(args)) || JsonEntitiesContainer.get("default");
...
class JsonLazyRef {
  ...
  get name() {
    return core.nameOf(this.getType());
  }
...
})(exports.SpecTypes || (exports.SpecTypes = {}));
...
function createRefName(name, options) {
  if (options.groups && options.groups.length) {
    return changeCase.pascalCase(`${name} ${options.groups.join(" ")}`);
  }
...
function createRef(name, schema, options) {
  ...
  if (schema.nullable) {
    switch (options.specType) {
      case exports.SpecTypes.OPENAPI:
        return {
          nullable: true,
          allOf: [ref]
        };

      case exports.SpecTypes.JSON:
        return {
          oneOf: [{
            type: "null"
          }, ref]
...
exports.JsonParameterTypes = void 0;
...
})(exports.JsonParameterTypes || (exports.JsonParameterTypes = {}));
/**
 * @ignore
 */
...
function isParameterType(type) {
  return Object.values(exports.JsonParameterTypes).includes(formatParameterType(type));
}
...
function mapHeaders(headers) {
  return Object.keys(headers).reduce((newHeaders, key) => {
    ...
    newHeaders[key] = core.cleanObject(options);
    return newHeaders;
  }, {});
}
...
class JsonResponse extends JsonMap {
  ...
  toJSON(options = {}) {
    ...
    if (options.specType === exports.SpecTypes.OPENAPI && response.headers) {
    ...
    if (options.specType !== exports.SpecTypes.OPENAPI && response.content) {
      const key = Object.keys(response.content)[0];
      return core.cleanObject(_extends({}, response, response.content[key], {
        content: undefined
      }));
    }
...
function getComputedType(target) {
  if (core.isPromise(target)) {
    return Object;
  }

  if (core.isClass(target) || core.isSymbol(target) || core.isPrimitiveOrPrimitiveClass(target) || core.isClassObject(target) || core.isDate(target) || core.isCollection(target)) {
    return core.classOf(target);
  }

  if (core.isArrowFn(target)) {
    return target();
  }
...
function getJsonType(value) {
  ...
  if (core.isPrimitiveOrPrimitiveClass(value)) {
...
function mapProperties(properties) {
  ...
  return Object.entries(properties).reduce((properties, [key, schema]) => {
    properties[toJsonRegex(key)] = core.isArray(schema) ? schema.map(mapToJsonSchema) : mapToJsonSchema(schema);
    return properties;
  }, {});
}
...
function mapToJsonSchema(item) {
  ...
  if (core.classOf(item) !== Object && core.isClass(item)) {
...
class JsonSchema extends Map {
  ...
  get isClass() {
    return core.isClass(this.class) && ![Map, Array, Set, Object, Date, Boolean, Number, String].includes(this._target);
  }
  /**
  ...
  enum(enumValues) {
    super.set("enum", core.uniq(enumValues));
    return this;
  }
  /**
  ...
  type(type) {
    switch (type) {
      ...
      default:
        if (core.isClass(type) || core.isFunction(type)) {
  ...
  any(...types) {
    types = core.uniq(types.length ? types : ["integer", "number", "string", "boolean", "array", "object", "null"]).map(getJsonType);
  ...
  set(key, value) {
    if (key in this) {
      core.isFunction(this[key]) && this[key](value);
    } else {
      super.set(key, value);
    }
  ...
  getName() {
    return this.get("name") || (this._target ? core.nameOf(core.classOf(this.getComputedType())) : "");
  }
...
class JsonOperation extends JsonMap {
  ...
  addTags(tags) {
    tags = core.uniqBy([...(this.get("tags") || []), ...tags], "name");
    ...
    if (!currentResponse) {
      ...
      response.forEach((value, key) => {
        if (!["content"].includes(key)) {
          currentResponse.set(key, core.deepMerge(currentResponse.get(key), value));
        }
      });
  ...
  addSecurityScopes(name, scopes) {
    ...
    security[name] = core.uniq([...(security[name] || []), ...scopes]);
  ...
  addProduce(produce) {
    const produces = core.uniq([].concat(this.get("produces"), produce)).filter(Boolean);
  ...
  toJSON(options = {}) {
    ...
    this.get("parameters").forEach(parameter => {
      if (!isParameterType(this.get("in"))) {
        if (parameter.get("in")) {
          if ([exports.JsonParameterTypes.BODY, exports.JsonParameterTypes.FILES].includes(parameter.get("in"))) {
    ...
    if (bodyParameters.length) {
      ...
      if (options.specType === exports.SpecTypes.OPENAPI) {
...
function toJsonParameter(parameter) {
  return _extends({
    in: exports.JsonParameterTypes.BODY,
    name: exports.JsonParameterTypes.BODY
  }, parameter);
}
...
class JsonParameter extends JsonMap {
  ...
  toJSON(options = {}) {
    ...
    const schemasContainer = core.toMap(options.schemas || {});
  ...
  build(options, schemasContainer) {
    ...
    parameter.required = parameter.required || this.get("in") === exports.JsonParameterTypes.PATH;

    if (this.get("in") === exports.JsonParameterTypes.FILES) {
      const isOpenApi = options.specType === exports.SpecTypes.OPENAPI;
      const schema = {
      ...
      if (jsonSchema.type === "array") {
        jsonSchema.items = core.cleanObject(_extends({}, jsonSchema.items, schema));
        parameter.schema = jsonSchema;
      } else {
        parameter.schema = core.cleanObject(_extends({}, jsonSchema, schema));
      }
    ...
    if (options.specType === exports.SpecTypes.SWAGGER) {
      ...
      if (["formData", "query"].includes(this.get("in"))) {
        ...
        if (jsonSchema.type === "array") {
          ...
          return core.cleanObject(_extends({}, parameter, props, {
            type: "array",
      ...
      if (this.get("in") !== "body") {
        return core.cleanObject(_extends({}, parameter, jsonSchema), IGNORE_OS2_PROPS);
      }
    }
  ...
  refToParameters(parameter, options, schemasContainer) {
    ...
    return Object.entries(schema.properties || {}).reduce((params, [key, _ref]) => {
      ...
      if (options.specType === exports.SpecTypes.OPENAPI) {
        return [...params, core.cleanObject(_extends({}, parameter, {
          name: key,
...
const getSchema = type => {
  if (core.isCollection(type) || !core.isClass(type)) {
...
class JsonEntityStore extends core.Entity {
  constructor(options) {
    super(options);
    this.isStore = true;
    /**
  ...
  get nestedGenerics() {
    switch (this.decoratorType) {
      case core.DecoratorTypes.PARAM:
  ...
  static fromMethod(target, propertyKey) {
    return this.from(target, propertyKey, core.descriptorOf(target, propertyKey));
  }
  ...
  build() {
    if (!this._type) {
      ...
      switch (this.decoratorType) {
        case core.DecoratorTypes.PARAM:
          type = core.Metadata.getParamTypes(this.target, this.propertyKey)[this.index];
          break;
        ...
        case core.DecoratorTypes.PROP:
          type = core.Metadata.getType(this.target, this.propertyKey);
          break;

        case core.DecoratorTypes.METHOD:
          type = core.Metadata.getReturnType(this.target, this.propertyKey);
          type = core.isPromise(type) ? undefined : type;
          break;
      }

      if (core.isCollection(type)) {
  ...
  createOperation() {
    ...
    if (core.isCollection(this._type)) {
...
function getInheritedStores(target) {
  ...
  if (!store.$inherited) {
    store.$inherited = core.ancestorsOf(store.target).reverse().reduce((context, model) => {
...
function inheritedClassMapper(obj, _ref = {}) {
  ...
  const stores = Array.from(getInheritedStores(target).entries()).filter(([model]) => core.classOf(model) !== core.classOf(target));

  if (stores.length) {
    const schema = stores.reduce((obj, [, store]) => {
      return core.deepMerge(obj, execMapper("schema", store.schema, options));
    }, {});
    obj = core.deepMerge(schema, obj);
  }
...
function extractRequiredProps(obj, schema, options) {
  ...
  return alterRequiredGroups(core.uniq(required), schema, options);
}
/**
 * @ignore
 */
...
function getRequiredProperties(obj, schema, options) {
  ...
  required = core.uniq(required).reduce(mapRequiredProps(obj, schema, options), []);
...
function mapNullableType(obj, schema, options) {
  ...
  switch (options.specType) {
    case exports.SpecTypes.SWAGGER:
    case exports.SpecTypes.JSON:
      obj.type = ["null", ...types];
      break;
...
const IGNORES = ["name", "$required", "$hooks", "_nestedGenerics", exports.SpecTypes.OPENAPI, exports.SpecTypes.SWAGGER, exports.SpecTypes.JSON];
/**
 * @ignore
 */
...
function shouldSkipKey(key, {
  specType = exports.SpecTypes.JSON,
  customKeys = false
}) {
  return IGNORES.includes(key) || key.startsWith("#") && (customKeys === false || specType !== exports.SpecTypes.JSON) || specType === exports.SpecTypes.SWAGGER && IGNORES_OS2.includes(key) || specType !== exports.SpecTypes.JSON && IGNORES_OPENSPEC.includes(key);
}
...
function schemaMapper(schema, options = {}) {
  ...
  let obj = [...schema.entries()].reduce((item, [key, value]) => {
    ...
    if (key === "examples" && core.isObject(value) && [exports.SpecTypes.OPENAPI, exports.SpecTypes.SWAGGER].includes(options.specType)) {
      key = "example";
...
function getJsonSchema(model, options = {}) {
  ...
  options = _extends({
    ...
    specType: options.specType || exports.SpecTypes.JSON,
    schemas: {}
  });
...
function mergeOperation(obj, operation, {
  ...
  const pathParameters = getJsonPathParameters(rootPath, path).map(({
    ...
    if (specType === exports.SpecTypes.OPENAPI) {
...
function operationIdFormatter(pattern = "") {
  ...
  const transform = (name, propertyKey) => {
    ...
    return pattern === "" ? changeCase.camelCase(str) : str;
  };
  ...
  return (name, propertyKey, path = "") => {
    ...
    if (result) {
      ...
      if (!OPERATION_IDS.has(operationKey)) {
        OPERATION_IDS.set(operationKey, 0);
        return changeCase.camelCase(`${operationId} By ${result}`);
      }
    }
...
function getSpec(model, options = {
  specType: exports.SpecTypes.SWAGGER
}) {
  if (!options.specType) {
    options.specType = exports.SpecTypes.SWAGGER;
  }
  ...
  return get(model, options, () => {
    ...
    const defaultTags = core.cleanObject({
    ...
    specJson.tags = core.uniqBy(tags, "name");
...
const schemesReducer = core.mergeReducerBuilder((current, value) => current.type && current.type === value.type || current.$ref && current.$ref === value.$ref);
/**
 * @ignore
 */

const parameters = core.mergeReducerBuilder((current, value) => current.in === value.in && current.name === value.name);
/**
 * @ignore
 */

const security = core.mergeReducerBuilder((current, value) => {
...
const tagsReducer = core.mergeReducerBuilder((current, value) => current.name === value.name);
/**
 * @ignore
 */
...
function mergeSpec(spec, input) {
  return core.deepMerge(spec, input, {
...
})(exports.JsonFormatTypes || (exports.JsonFormatTypes = {}));
...
function date() {
  return from(Date).format(exports.JsonFormatTypes.DATE);
}
/**
...
function datetime() {
  return from(Date).format(exports.JsonFormatTypes.DATE_TIME);
}
/**
...
function time() {
  return from(Date).format(exports.JsonFormatTypes.TIME);
}
/**
...
function email() {
  return from(String).format(exports.JsonFormatTypes.EMAIL);
}
/**
...
function uri() {
  return from(String).format(exports.JsonFormatTypes.URI);
}
/**
...
function url() {
  return from(String).format(exports.JsonFormatTypes.URL);
}
/**
...
function isEnum$1(type) {
  return core.isObject(type) && !("toJSON" in type);
}
/**
...
function Deprecated(deprecated = true) {
  return JsonEntityFn((store, args) => {
    switch (store.decoratorType) {
      ...
      case core.DecoratorTypes.CLASS:
        core.decorateMethodsOf(args[0], Deprecated(deprecated));
        break;

      default:
        throw new core.UnsupportedDecoratorType(Deprecated, args);
    }
  });
}
...
function Consumes(...consumes) {
  return JsonEntityFn((store, args) => {
    switch (store.decoratorType) {
      ...
      case core.DecoratorTypes.CLASS:
        core.decorateMethodsOf(args[0], Consumes(...consumes));
        break;

      default:
        throw new core.UnsupportedDecoratorType(Consumes, args);
    }
  });
}
...
class DecoratorContext extends Map {
  ...
  build() {
    ...
    this.methods.forEach(name => {
      decorator[changeCase.pascalCase(name)] = wrapKey(name);
    });
    return decorator;
  }
  ...
  getMergedKey(key, defaultValue) {
    ...
    if (this.decoratorType === core.DecoratorTypes.CLASS) {
...
class ReturnDecoratorContext extends DecoratorContext {
  ...
  examples(examples) {
    this.set("examples", core.isString(examples) ? [examples] : examples);
    return this;
  }
  ...
  checkPrimitive(model) {
    if (core.isPrimitiveOrPrimitiveClass(model)) {
  ...
  getContentType() {
    ...
    if (model && !core.isPlainObject(model) && !core.isPrimitiveOrPrimitiveClass(model)) {
  ...
  onInit(args, decorator) {
    const type = core.decoratorTypeOf(args);
    ...
    switch (type) {
      ...
      case core.DecoratorTypes.CLASS:
        this.decoratorType = core.DecoratorTypes.CLASS;
        core.decorateMethodsOf(args[0], decorator);
        break;

      default:
        throw new core.UnsupportedDecoratorType(Returns, args);
    }
  }
  ...
  map() {
    ...
    if (description) {
      description = decoratorType === core.DecoratorTypes.CLASS ? response.get("description") || description : description;
...
function In(inType) {
  ...
  const decorator = (target, propertyKey, index) => {
    ...
    switch (store.decoratorType) {
      ...
      case core.DecoratorTypes.CLASS:
        core.decorateMethodsOf(target, decorator);
        break;

      default:
        throw new core.UnsupportedDecoratorType(In, [target, propertyKey, index]);
    }
  };
...
function OperationId(operationId) {
  return JsonEntityFn((store, args) => {
    if (store.decoratorType !== core.DecoratorTypes.METHOD) {
      throw new core.UnsupportedDecoratorType(OperationId, args);
    }
...
exports.OperationMethods = void 0;
...
})(exports.OperationMethods || (exports.OperationMethods = {}));
/**
...
function OperationPath(method, path = "/") {
  return JsonEntityFn((store, args) => {
    if (store.decoratorType !== core.DecoratorTypes.METHOD) {
      throw new core.UnsupportedDecoratorType(OperationPath, args);
    }
...
function Path(path) {
  return JsonEntityFn((store, args) => {
    if (store.decoratorType !== core.DecoratorTypes.CLASS) {
      throw new core.UnsupportedDecoratorType(Path, args);
    }
...
function Produces(...produces) {
  return JsonEntityFn((store, args) => {
    switch (store.decoratorType) {
      ...
      case core.DecoratorTypes.CLASS:
        core.decorateMethodsOf(args[0], Produces(...produces));
        break;

      default:
        throw new core.UnsupportedDecoratorType(Produces, args);
    }
  });
}
...
function Security(name, ...scopes) {
  return JsonEntityFn((store, args) => {
    switch (store.decoratorType) {
      ...
      case core.DecoratorTypes.CLASS:
        core.decorateMethodsOf(args[0], Security(name, ...scopes));
        break;

      default:
        throw new core.UnsupportedDecoratorType(Security, args);
    }
  });
}
...
function Tags(...tags) {
  return JsonEntityFn((store, args) => {
    switch (store.decoratorType) {
      ...
      case core.DecoratorTypes.CLASS:
        core.decorateMethodsOf(args[0], Tags(...tags));
        break;

      default:
        throw new core.UnsupportedDecoratorType(Tags, args);
    }
  });
}
...
function Summary(summary) {
  return JsonEntityFn((store, args) => {
    if (store.decoratorType !== core.DecoratorTypes.METHOD) {
      throw new core.UnsupportedDecoratorType(Summary, args);
    }
...
function matchGroups(groups, compareWith = []) {
  ...
  if (groupsExcludes.length) {
    if (compareWith.length && micromatch__default['default'](groupsExcludes, compareWith).length) {
      return true;
    }
  }

  if (groupsIncludes.length) {
    return !micromatch__default['default'](groups.filter(group => !group.startsWith("!")), compareWith).length;
  }
...
function Allow(...values) {
  const model = values.find(item => core.isClass(item));
  return core.useDecorators(model && Property(model), JsonEntityFn((store, args) => {
    ...
    switch (store.decoratorType) {
      ...
      default:
        throw new core.UnsupportedDecoratorType(Allow, args);
    }
  }));
}
...
function Description(description) {
  return JsonEntityFn((store, args) => {
    ...
    switch (core.decoratorTypeOf(args)) {
      ...
      default:
        throw new core.UnsupportedDecoratorType(Description, args);
    }
  });
}
...
function Example(...examples) {
  return JsonEntityFn((store, args) => {
    switch (core.decoratorTypeOf(args)) {
      ...
      default:
        throw new core.UnsupportedDecoratorType(Example, args);
    }
  });
}
...
function Enum(enumValue, ...enumValues) {
  return JsonEntityFn(store => {
    const values = [enumValue].concat(enumValues).reduce((acc, value) => {
      if (core.isObject(value) && value !== null) {
...
function Email() {
  return Format(exports.JsonFormatTypes.EMAIL);
}
/**
...
function DateTime() {
  return Format(exports.JsonFormatTypes.DATE_TIME);
}
/**
...
function DateFormat() {
  return Format(exports.JsonFormatTypes.DATE);
}
/**
 * Apply a time validation on property.
 *
...
function TimeFormat() {
  return Format(exports.JsonFormatTypes.TIME);
}
/**
...
function Uri() {
  return Format(exports.JsonFormatTypes.URI);
}
/**
...
function Url() {
  return Format(exports.JsonFormatTypes.URL);
}
...
function Optional() {
  return JsonEntityFn((store, args) => {
    switch (store.decoratorType) {
      ...
      default:
        throw new core.UnsupportedDecoratorType(Optional, args);
    }
  });
}
...
function RequiredGroups(...groups) {
  return JsonEntityFn(entity => {
    if (entity.decoratorType === core.DecoratorTypes.PROP) {
...
function Hidden() {
  return core.StoreSet("hidden", true);
}
...
function Nullable(type, ...types) {
  types = [type, ...types];
  return core.useDecorators(types.length === 1 && Property(types[0]), Any(null, ...types));
}
...
exports.schemesReducer = schemesReducer;
exports.security = security;
exports.set = set;
