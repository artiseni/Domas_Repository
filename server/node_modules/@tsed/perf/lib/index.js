...
class PerfLogger {
  ...
  bind(instance) {
    const methods = core.methodsOf(core.classOf(instance));
    const {
      wrap,
      log
    } = this;
    methods.forEach(({
      ...
      const descriptor = core.descriptorOf(target, propertyKey);
      const name = core.nameOf(target);
  ...
  formatDiff(diff, prefix = "") {
    ...
    return chalk__default['default'][color](label);
  }
