...
const defaultKeyResolver = args => {
  return args.map(arg => core.isClass(arg) ? JSON.stringify(jsonMapper.serialize(arg)) : arg).join(":");
};
/**
 * @platform
 */
...
exports.PlatformCache = class PlatformCache {
  disabled() {
  ...
  $onInit() {
    ...
    if (settings) {
      ...
      this.cache = caches != null && caches.length ? cacheManager__default['default'].multiCaching(caches, _extends({}, props)) : cacheManager__default['default'].caching(_extends({}, props, {
        ttl,
        store
      }));
    }
  ...
  ttl(result, currentTtl) {
    ...
    return core.isFunction(ttl) ? ttl(result) : ttl;
  }
  ...
  async get(key, options = {}) {
    ...
    return jsonMapper.deserialize((_this$cache2 = this.cache) == null ? void 0 : _this$cache2.get(key), options);
  }
  ...
  async getMatchingKeys(patterns) {
    ...
    return micromatch__default['default'](keys, patterns);
  }
...
tslib.__decorate([di.Configuration(), tslib.__metadata("design:type", Object)], exports.PlatformCache.prototype, "settings", void 0);

exports.PlatformCache = tslib.__decorate([di.Injectable()], exports.PlatformCache);
...
let PlatformCacheInterceptor = class PlatformCacheInterceptor {
  ...
  getArgs(context) {
    return context.args.reduce((args, arg) => {
      if (arg instanceof di.DIContext || arg instanceof http.IncomingMessage || arg instanceof http.ServerResponse) {
        return args;
      }

      if (core.isClass(arg)) {
        return args.concat(jsonMapper.serialize(arg));
      }
  ...
  getOptions(context) {
    ...
    const keyArgs = core.isString(k) ? k : k(args);
    return {
      ttl,
      type,
      args,
  ...
  isEndpoint({
    ...
    return core.Store.fromMethod(target, propertyKey).has(schema.JsonEntityStore);
  }
  ...
  async cacheMethod(context, next) {
    ...
    const key = [core.nameOf(context.target), context.propertyKey, keyArgs].join(":");
    ...
    if (cachedObject) {
      ...
      return jsonMapper.deserialize(JSON.parse(data), {
        collectionType,
        type
      });
    }
    ...
    await this.cache.set(key, {
      ttl: calculatedTtl,
      args,
      data: JSON.stringify(jsonMapper.serialize(result, {
        type,
        collectionType
      }))
    }, {
...
tslib.__decorate([di.Inject(), tslib.__metadata("design:type", exports.PlatformCache)], PlatformCacheInterceptor.prototype, "cache", void 0);

PlatformCacheInterceptor = tslib.__decorate([di.Interceptor()], PlatformCacheInterceptor);

function UseCache(options = {}) {
  return core.useDecorators(di.Intercept(PlatformCacheInterceptor, options));
}
...
exports.PlatformCacheModule = class PlatformCacheModule {};
exports.PlatformCacheModule = tslib.__decorate([di.Module({
  imports: [exports.PlatformCache]
})], exports.PlatformCacheModule);
